...
// Other common placeholder
['Modal', 'Loader', 'Pagination', 'SearchBar', 'StatusBadge', 'EmptyState'].forEach(name => {
  write(\`src/components/common/\${name}.jsx\`, \`
import React from 'react';
...
// Charts Placeholders
['BarChart', 'LineChart', 'PieChart', 'AreaChart'].forEach(name => {
  write(\`src/components/charts/\${name}.jsx\`, \`
import React from 'react';
...
// Dummy pages generator function
const generatePage = (name, role) => \`
import React from 'react';
  ...
  return (
    <DashboardLayout>
      <div className="page-header">
        <h2>\${role} / \${name}</h2>
        <p className="breadcrumb">Home > \${role} > \${name}</p>
      </div>
...
Object.entries(pages).forEach(([role, pageList]) => {
  pageList.forEach(page => {
    write(\`src/pages/\${role}/\${page}.jsx\`, generatePage(page, role));
  });
});

// Auth Pages (override layout for login)
...
// App.jsx and main.jsx and routing
write('src/App.jsx', \`
import React from 'react';
...
console.log("Successfully generated all code content!");
